...
namespace QTRHacker.PlayerEditor
{
	public class PlayerEditor : TabPage
	{
		...
		private void InitData()
		{
			...
			HairColorControl.Color = ColorFromRedef(TargetPlayer.HairColor);
			SkinColorControl.Color = ColorFromRedef(TargetPlayer.SkinColor);
			EyeColorControl.Color = ColorFromRedef(TargetPlayer.EyeColor);
			ShirtColorControl.Color = ColorFromRedef(TargetPlayer.ShirtColor);
			UnderShirtColorControl.Color = ColorFromRedef(TargetPlayer.UnderShirtColor);
			PantsColorControl.Color = ColorFromRedef(TargetPlayer.PantsColor);
			ShoesColorControl.Color = ColorFromRedef(TargetPlayer.ShoeColor);

			HealthTextBox.Text = TargetPlayer.StatLifeMax.ToString();
			ManaTextBox.Text = TargetPlayer.StatManaMax.ToString();
		}

		private void ApplyData()
		{
			TargetPlayer.Hair = (int)HairStyleControl.Value;
			TargetPlayer.HairColor = RedefFromColor(HairColorControl.Color);
			TargetPlayer.SkinColor = RedefFromColor(SkinColorControl.Color);
			TargetPlayer.EyeColor = RedefFromColor(EyeColorControl.Color);
			TargetPlayer.ShirtColor = RedefFromColor(ShirtColorControl.Color);
			TargetPlayer.UnderShirtColor = RedefFromColor(UnderShirtColorControl.Color);
			TargetPlayer.PantsColor = RedefFromColor(PantsColorControl.Color);
			TargetPlayer.ShoeColor = RedefFromColor(ShoesColorControl.Color);

			TargetPlayer.StatLifeMax = Convert.ToInt32(HealthTextBox.Text);
			TargetPlayer.StatManaMax = Convert.ToInt32(ManaTextBox.Text);
		}

		public static Microsoft.Xna.Framework.Color ColorFromRedef(Functions.GameObjects.ValueTypeRedefs.Xna.Color color) => new(color.R, color.G, color.B, color.A);
		public static Functions.GameObjects.ValueTypeRedefs.Xna.Color RedefFromColor(Microsoft.Xna.Framework.Color i) => new() { PackedValue = i.PackedValue };
	}
...
#pragma warning restore CA2245
	...
	public class PlayerView : GraphicsDeviceControl
	{
		...
		protected override void Initialize()
		{
			...
			using (var s = Assembly.GetExecutingAssembly().GetManifestResourceStream("QTRHacker.Res.ContentImage.PlayerModelsRaw.bin"))
			{
