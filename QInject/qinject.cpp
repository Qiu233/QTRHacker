	...
	while ((status = pEnumUnknown->Next(1, &pCLRRuntimeInfoThunk, &fetched)) == S_OK && fetched == 1) {
		...
		while ((status = pCorRuntimeHost->NextDomain(domainEnum, &pAppDomainThunk)) == S_OK) {
			...
			if (!options.enumerate && (!options.appDomainIndex || options.appDomainIndex == appDomainIndex)) {
				...
				arr.cDims = 1;
				arr.cbElements = 1;
				arr.rgsabound[0].cElements = options.assemblySize;
				arr.rgsabound[0].lLbound = 0;
				arr.pvData = options.assembly;
				mscorlib::_Assembly *assembly = NULL;
				status = pAppDomain->Load_3(&arr, &assembly);
				REMOTE_ERROR_STATUS(8, "_AppDomain->Load_3 failed!");
				VARIANT v;
				status = assembly->CreateInstance(typeName, &v);
				REMOTE_ERROR_STATUS(8, "assembly->CreateInstance failed!");
...
void Inject(DWORD processId, void*assembly, int assemblySize, OLECHAR* typeName) {
	...
	lstrcpyW(options.typeName, typeName);
	InjectionResult result;
	Inject(&options, &result);
}